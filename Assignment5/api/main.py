...
def read_one_sandwich(sandwich_id: int, db: Session = Depends(get_db)):
    db_sandwich = sandwiches_controller.read_one(db, sandwich_id=sandwich_id)
...
def update_one_sandwich(sandwich_id: int, sandwich: schemas.SandwichUpdate, db: Session = Depends(get_db)):
    ...
    if db_sandwich is None:
        raise HTTPException(status_code=404, detail="Sandwich not found")
    return sandwiches_controller.update(db=db, sandwich_id=sandwich_id, sandwich=sandwich)
